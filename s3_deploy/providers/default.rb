...
action :create do
  ...
  deploy_root = new_resource.deploy_root ? new_resource.deploy_root : node['s3_deploy']['deploy_root']
  builds_bucket = new_resource.bucket ? new_resource.bucket : node['s3_deploy']['bucket']
  ...
  raise "You need to set at least the [:s3_deploy][:bucket] attribute if the bucket attribute of the lwrp is empty" if builds_bucket.empty?
